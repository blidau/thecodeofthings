...
# tags
class PostPageTag(taggit_models.TaggedItemBase):
    """
    A tag for a post.
    """

    content_object = modelcluster_fields.ParentalKey(
        "blog.PostPage", related_name="tagged_posts", on_delete=models.CASCADE
    )
...
class SectionBlock(wagtail_blocks.StructBlock):
    ...
    class Meta:
        template = "blog/blocks/section.html"
...
class PostPage(wagtail_models.Page):
    """
    A standard post.
    """
    ...
    tags = modelcluster_taggit.ClusterTaggableManager(through="blog.PostPageTag", blank=True)
    categories = modelcluster_fields.ParentalManyToManyField("blog.Category", blank=True)
    body = wagtail_fields.StreamField(
        [
            ("heading", wagtail_blocks.CharBlock()),
            ("paragraph", wagtail_blocks.RichTextBlock()),
